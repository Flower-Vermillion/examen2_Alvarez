package co.edu.poli.examen2_Alvarez.controlador;
...
import co.edu.poli.examen2_Alvarez.servicios.DAOSeguro;
import co.edu.poli.examen2_Alvarez.servicios.DAOAsegurado;
...
public class ControlFormSecure {
	...
	@FXML
	private RadioButton radio1; // SeguroDeVida
	@FXML
	private RadioButton radio2; // SeguroDeVehiculo
	@FXML
	...
	private void pressConsulta(ActionEvent event) {
		...
		if (!txtSeguro1.getText().isBlank() || !txtSeguro1.getText().isEmpty()) {
			try {
				Seguro t = daoSeguro.readone(txtSeguro1.getText());

				if (t != null)
					txtAreaResultado.setText(t.toString());
				else
					mostrarAlerta("No existe el numero del seguro");
			} catch (Exception e) {
				mostrarAlerta(e.getMessage());
			}
		} else
			mostrarAlerta("Ingrese número del seguro");
	}
	...
	private void pressCreacion(ActionEvent event) {
		...
		if (radio1.isSelected()) {
			nueva = new SeguroDeVida(numero, fechaExp, true, asegurado, true);
		} else {
			nueva = new SeguroDeVehiculo(numero, fechaExp, true, asegurado, txtMarca.getText());
		}
